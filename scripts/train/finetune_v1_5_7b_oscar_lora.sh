...
deepspeed --master_port "${MASTER_PORT:-29700}" "$REPO_ROOT/llava/train/train_mem.py" \
    ...
    --pretrain_mm_mlp_adapter "${MM_PROJECTOR_PATH:-$REPO_ROOT/checkpoints/llava-v1.5-7b-pretrain/mm_projector.bin}" \
    ...
    --bf16 True \
    --output_dir "${OUTPUT_DIR:-$REPO_ROOT/checkpoints/llava-v1.5-7b-lora-oscar}" \
    --num_train_epochs 1 \
