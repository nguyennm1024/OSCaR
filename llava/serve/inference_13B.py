...
def main(args):
    ...
    for i, image_path in enumerate(file_paths):
        ...
        if 'state_change' in resolved_path:
            inp = "Describe the state change and progress of the <object> through these frames".replace('<object>', object_names[i])
        else:
            inp = "Describe state of the <object> in this image in a detailed way".replace('<object>', object_names[i])
        ...
        output_path = Path(args.output_root) / model_name_part / output_relative_path(resolved_path)
